...
Проверяется каждое из этого:
  ...
  * **дополнение цен понемногу.** Выключатель на виду, и пока он выключен,
    наружу не уходит ни одного запроса. Здесь проверяется именно это: что
    выключатель есть, что он переключается и что по умолчанию всё выключено.
...
with sync_playwright() as pw:
    ...
    watch = get("/api/prices/auto")
    check("по умолчанию выключено", not watch["running"], str(watch["running"]))
    check("но видно, сколько осталось спросить",
          isinstance(watch.get("left"), int),
          "%s назв." % watch.get("left"))
    check("пауза между запросами настоящая", watch["pause"] >= 10,
          "%s с" % watch["pause"])
    check("выключатель показан", page.locator("#coll-watch").count() == 1)

    page.check("#coll-watch")
    page.wait_for_timeout(900)
    check("включается", get("/api/prices/auto")["running"])
    page.uncheck("#coll-watch")
    page.wait_for_timeout(900)
    check("и выключается", not get("/api/prices/auto")["running"])
